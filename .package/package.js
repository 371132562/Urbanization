...
async function main() {
  try {
    ...
    execSync('pnpm --filter urbanization-backend exec prisma generate', {
      cwd: rootDir,
      stdio: 'inherit'
    })
    ...
    console.log('📦 正在为后端安装生产依赖 (pnpm install)...')
    execSync('pnpm install --prod', { cwd: backendAppDir, stdio: 'inherit' })
    console.log('✅ 后端依赖安装完成。')

    // 新增: 确保 Prisma 引擎和依赖被正确复制
    console.log('🔄 正在确保 Prisma 依赖完整...')
    execSync('pnpm add @prisma/client prisma', { cwd: backendAppDir, stdio: 'inherit' })
    execSync('npx prisma generate', { cwd: backendAppDir, stdio: 'inherit' })
    console.log('✅ Prisma 依赖确认完成。')
